...
def test_should_have_correct_url():
    g = Gdax()
    assert b.api_base == 'https://api.gdax.com'
...
def test_should_return_ticker():
    ...
    mock_url = API_BASE + 'products/' + mock_symbol + '/ticker'
    mock_status = 200
...
def test_should_return_orderbook():
    ...
    mock_symbol = 'btcusd'
    mock_body = (
            '{"sequence": "3","bids": [["295.96","4.39088265", 2 ]],"asks": ' +
            '[["295.97", "25.23542881", 12 ]]}'
            )
    mock_url = API_BASE + 'products/' + mock_symbol + '/book?level=2'
    mock_status = 200
